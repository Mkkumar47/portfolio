...
const MOBILE_SOCIAL_ICONS: Record<string, ReactElement> = {
...
export default function PublicationsFooterSection() {
  ...
  useEffect(() => {
    ...
    type VidUniforms = {
      uVideo: { value: THREE.VideoTexture }
      uOpacity: { value: number }
      uVideoAspect: { value: number }
      uCanvasAspect: { value: number }
    }

    let renderer: THREE.WebGLRenderer | undefined
    let vidUni: VidUniforms | undefined
    ...
    if (!isMobile && canvas && videoEl) {
      ...
      videoEl.src       = '/assets/about-me.mp4'
      videoEl.muted     = true
          ...
          <div ref={bottomBarRef} className={styles.bottomBar}>
            <div className={styles.bottomLeft}>
              <div className={styles.monogram}>
                <span className={styles.monoLetters}>MK</span>
                <span className={styles.monoDot} />
              </div>
              <span className={styles.leftDivider} />
              <div className={styles.copyBlock}>
                <p className={styles.copy}>© {year} {profile.name.nickname.toUpperCase()}</p>
