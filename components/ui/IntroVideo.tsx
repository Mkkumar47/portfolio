...
export default function IntroVideo() {
  ...
  function dismiss() {
    ...
    // Audio belongs to the intro only — silence it the moment we leave,
    // so no sound plays over the home page during/after the fade-out.
  ...
  useEffect(() => {
    ...
    const v = videoRef.current
    const UNLOCK_EVENTS: (keyof WindowEventMap)[] = ['pointerdown', 'touchstart', 'keydown', 'click']
    let unlockAudio: (() => void) | undefined
    if (v) {
      const enableAudio = () => {
        // Never (re)enable sound once the intro has been dismissed.
        if (dismissedRef.current) return
    ...
    return () => {
      ...
      if (handler) {
        UNLOCK_EVENTS.forEach((ev) => window.removeEventListener(ev, handler))
      }
